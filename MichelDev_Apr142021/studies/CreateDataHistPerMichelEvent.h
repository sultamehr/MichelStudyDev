...
class CreateDataHistPerMichelEvent: public Study

{
  public:
    using reco_t = std::function<double(const CVUniverse&, const MichelEvent&)>;
    CreateDataHistPerMichelEvent(reco_t reco, const std::string& varName, const std::string& varUnits, const int nBins, const double minBin, const double maxBin, std::vector<CVUniverse*>& univs): Study(), fReco(reco)
    {
